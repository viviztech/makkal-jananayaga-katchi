    ...
    <section class="py-12 lg:py-16 px-4 bg-gray-50 dark:bg-gray-900">
        <div class="max-w-7xl mx-auto">
            <div class="flex flex-col lg:flex-row gap-8">
                {{-- Main Kalaththil Siruthaigal Content --}}
                    ...
                    <div class="relative bg-white dark:bg-gray-800 rounded-lg overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 flex flex-col h-full border border-gray-200 dark:border-gray-700">
                        ...
                        <div class="relative h-48 overflow-hidden bg-gray-100 dark:bg-gray-700">
                             <a href="{{ route('media.show', $kalam->slug) }}" class="block w-full h-full">
                                <img src="{{ $kalam->featured_image_url ?? asset('assets/images/backgrounds/kalththil-siruthaigal.webp') }}" alt="{{ app()->getLocale() === 'ta' ? $kalam->title_ta : $kalam->title_en }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110">
                            </a>

                            {{-- Event Date Badge --}}
                            @if($kalam->event_date)
                            <div class="absolute top-2 right-2">
                                <div class="bg-white dark:bg-gray-900 text-teal-700 dark:text-teal-300 px-2 py-1 rounded-md text-xs font-semibold shadow-md">
                                    {{ $kalam->event_date->format('M j') }}
                                </div>
                             </div>
                            @endif
                        ...
                        <div class="p-4 flex flex-col flex-grow bg-white dark:bg-gray-800">
                            ...
                            <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100 mb-2 line-clamp-2 leading-tight min-h-[2.5rem]">
                                 <a href="{{ route('media.show', $kalam->slug) }}" class="hover:text-teal-600 dark:hover:text-teal-400 transition-colors duration-200">
                                    {!! app()->getLocale() === 'ta' ? $kalam->title_ta : $kalam->title_en !!}
                                </a>
                            </h3>

                            {{-- Description/Excerpt (Optional) --}}
                            @if(app()->getLocale() === 'ta' ? $kalam->content_ta : $kalam->content_en)
                            <p class="text-gray-600 dark:text-gray-300 text-xs mb-3 line-clamp-2 flex-grow leading-relaxed">
                                {!! Str::limit(strip_tags(app()->getLocale() === 'ta' ? $kalam->content_ta : $kalam->content_en), 80) !!}
                            </p>
                            @endif
                            ...
                            <div class="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mt-auto pt-3 border-t border-gray-200 dark:border-gray-700">
                                <span class="flex items-center">
                                    ...
                                    {{ $kalam->created_at->diffForHumans() }}
                                </span>
                                <span class="bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 px-2 py-0.5 rounded-full text-xs font-medium">
                                    {{ $kalam->category->name ?? 'Kalam' }}
                                </span>
                            </div>

                             {{-- Action Buttons --}}
                            <div class="mt-3 flex items-center gap-2">
                                <a href="{{ route('media.show', $kalam->slug) }}" class="flex-1 inline-flex items-center justify-center bg-teal-600 hover:bg-teal-700 text-white px-3 py-2 rounded-md text-xs font-medium transition-colors duration-200">
