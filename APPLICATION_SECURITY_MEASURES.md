# Application Form Security Measures

This document outlines all security measures implemented for the public application submission form to protect against spam, bots, and malicious submissions.
...
- **Location**: `resources/views/pages/applications.blade.php` (line 38)
...
- **Location**:
  - Form: `resources/views/pages/applications.blade.php` (line 41)
  - Validation: `app/Http/Requests/ApplicationRequest.php` (line 49)
  - Controller check: `app/Http/Controllers/PageController.php` (line 835)
...
- **Location**:
  - Form: `resources/views/pages/applications.blade.php` (line 44)
  - Validation: `app/Http/Controllers/PageController.php` (line 841)
...
- **Location**: `routes/web.php` (line 65-66)
...
- **Location**: `app/Http/Requests/ApplicationRequest.php` (line 21-37)
...
- **Note**: Same email can apply for multiple posts, but not the same post twice

#### Aadhar Number Validation
- **Pattern**: `/^[2-9][0-9]{11}$/`
- **Requirements**: Valid 12-digit Aadhar format
- **Sanitization**: Removes all non-numeric characters
- **Note**: Same Aadhar can apply for multiple posts, but not the same post twice

#### Voter ID Validation
- **Pattern**: `/^[A-Z]{3}[0-9]{7}$/`
- **Requirements**: 3 uppercase letters + 7 digits
- **Sanitization**: Converts to uppercase
- **Note**: Same Voter ID can apply for multiple posts, but not the same post twice
...
- **Storage**: Stored in `storage/app/public/applications/photos`

#### Document Upload
- **Allowed Types**: PDF, JPG, JPEG, PNG
- **Max Size**: 5MB
- **MIME Validation**: Validates actual file type
- **Storage**: Stored in `storage/app/public/applications/documents`

### 8. **Duplicate Submission Prevention** ✅

**Important**: One person can apply for multiple different posts, but cannot submit duplicate applications for the same post.

#### Post-Specific Duplicate Prevention
- **Check**: Prevents same person from applying for the same post multiple times
- **Detection Methods**:
  - Email-based: Checks if email already applied for this specific post
  - Mobile-based: Checks if mobile already applied for this specific post
  - Aadhar-based: Checks if Aadhar number already applied for this specific post
  - Voter ID-based: Checks if Voter ID already applied for this specific post
- **Protection**: Allows same person to apply for different posts, prevents duplicate applications for same post
- **Location**: `app/Http/Controllers/PageController.php` (line 888-932)
...
- **Location**: `resources/views/pages/applications.blade.php` (line 623-666)

### 10. **Date Validation** ✅
- **DOB**: Must be before today, after 1940
- **Joining Date**: Must be before or equal today, after 1980
- **Protection**: Prevents invalid or future dates

## 🚨 Additional Security Features

### Database-Level Protection
- **Soft Deletes**: Email uniqueness check excludes deleted records
- **Foreign Key Constraints**: Validates all location/position references
- **Guarded Fields**: Uses `$guarded = []` with explicit validation
...
- **Location**: `resources/views/pages/applications.blade.php` (line 669)
...
- **How to Add**: Send verification email before processing application
...
curl -X POST http://your-domain/applications \
  -d "website=test" \
  -d "name=Test" \
  # ... other fields
# Expected: 422 error
...
for i in {1..4}; do
  curl -X POST http://your-domain/applications -d "name=Test$i"
done
...
- **Duplicate Email**: "An application with this email was already submitted in the last 24 hours."
...
- Monitor application count per day for anomalies
